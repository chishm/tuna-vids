...
static bool cue_video(void)
{
    ...
    do {
        ...
        do {
            ...
            used_bytes = dec_main(nextFrameData.start, frameBuffer,
                nextFrameData.size, &xvid_dec_stats);
            if (used_bytes >= 0) {
                aviVideoUsedAmount(used_bytes);
            }
        } while (xvid_dec_stats.type <= 0
            && aviBuffer.videoAmountLeft >= MIN_USEFUL_BYTES);
        vidBuf_CurrentDecodeComplete();
    } while (vidBuf_NumFreeDecodeBuffers() > 0
        && aviBuffer.videoAmountLeft >= MIN_USEFUL_BYTES
        && nextFrameData.size > 0);
...
static void seekVideo(int offset, bool paused)
{
    ...
    flush_video();
    /* Jump to desired location in file. There are 2 chunks per frame, one for
     * audio, one for video */
    aviSeekOffset(offset);
    /* Start playing video, but it won't actually play until the audio is
     * started too. */
    vidBuf_ClearFutureBuffers();
    cue_video();
    mp3PlayerStart();
...
int play_movie(FILE* aviFile)
{
    ...
    /* Now that it's passed all points where an error will be reported,
     * initialise the GUI, removing the text display. */
    controlsSetup();
    ...
    while (videoPlaying && aviBuffer.videoAmountLeft >= MIN_USEFUL_BYTES
        && (paused || !mp3PlayerHasEnded()))
    {
        ...
        if ((keys & KEY_LEFT) || hitCheck(&penPos, &controlFpsMinus, keys)) {
            ...
            || hitCheck(&penPos, &controlFpsPlus, keys)) {
            // Skip a frame
            ++audioDelay;
            vidBuf_AdjustDelay(audioDelay);
            updateSyncDisplay(audioDelay);
        } else if ((keys & KEY_B)
            || hitCheck(&penPos, &controlPausePlay, keys)) {
            if (paused) {
                vidBuf_ResumeVideo();
                mp3PlayerPlay(true);
                showPause();
            ...
            seekVideo(penPos.px * (AVI_SEEK_OFFSET_FACTOR / SCREEN_WIDTH),
                paused);
        ...
        if ((vidBuf_vblanks - oldVblanks >= UPDATE_PERIOD) || keys != 0) {
            ...
            updateTimeDisplay(
                (framesShownOffset + videoStats.framesShown) / frameRateHz,
                videoStats.avgFrameRate, videoStats.framesBehind);
            updateSeekBar(
                256 * (framesShownOffset + videoStats.framesShown) / numFrames);
        }
    };
