...
static volatile int vidBuf_sampleRate = 44100;// Reasonable default (44.1kHz)
static int vidBuf_frameRate = 1000; // Reasonable default (10fps)
static volatile int vidBuf_soundSampleCount = 0;
static volatile int vidBuf_videoFrameCount = 0;
static volatile int vidBuf_running = false;
static volatile int vidBuf_videoFramesBehind = 0;
...
int vidBuf_NumFramesAvailable(void)
{
    ...
    int usedDecodeBuffers = videoPages[0].readyForDisplay
        + videoPages[1].readyForDisplay + videoPages[2].readyForDisplay;
...
{
    return NUM_VIDEO_PAGES - vidBuf_NumFramesAvailable();
}
...
void vidBuf_VblankHandler(void)
{
    ...
    delay = ((int64_t) vidBuf_soundSampleCount * (int64_t) vidBuf_frameRate)
        - ((int64_t) (vidBuf_videoFrameCount - vidBuf_audioDelay)
            * (int64_t) vidBuf_sampleRate * 100);
